"""
RepoEngineer — Smart File Selector
Scores every file and returns ALL valid code files, sorted by priority.
No arbitrary cap — fetches everything the repo has.
"""
...
def score_file(path: str) -> int:
    ...
    if re.match(
        ...
        r"application|program|init|__init__|__main__)$", fname_no_ext,
    ):
        score += 40
...
def select_files(all_files: list[str], limit: int = MAX_FILES_TO_FETCH) -> list[str]:
    """
    Return ALL valid source files sorted by priority score.
    No hard cap unless limit is explicitly set (default is 9999 = effectively unlimited).
    """
    ...
    for path in all_files:
        ...
        scored.append(ScoredFile(
            path=path, score=s, ext=_get_ext(path),
            depth=path.count("/"), is_test=bool(_TEST_RE.search(path.lower())),
        ))
...
def stats_summary(all_files: list[str]) -> dict:
    ...
    code_files = [f for f in all_files
                  if _get_ext(f) in CODE_EXTENSIONS and not _SKIP_RE.search(f)]
    return {
        "total":      len(all_files),
        "code":       len(code_files),
